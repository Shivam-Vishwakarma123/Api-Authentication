...
exports.login = function (req, res) {
  ...
  if (req.body.constructor === Object && Object.keys(req.body).length === 0) {
    res
      .status(400)
      .send({ error: true, message: "Please provide correct credentials" });
  } else {
    User.login(new_user, function (err, User) {
      ...
      bcrypt.compare(new_user.password, User.password, (err, isMatch) => {
        if (err) {
          return res
            .status(500)
            .send({ error: true, message: "Error comparing passwords" });
        }
...
exports.register = function (req, res) {
  ...
  if (!newUser.user_name || !newUser.password) {
    return res
      .status(400)
      .send({ error: true, message: "Please provide user name & password" });
  }
        ...
        bcrypt.hash(newUser.password, salt, function (err, hash) {
          ...
          User.register(newUser, function (err, userId) {
            if (err) {
              return res
                .status(500)
                .send({ error: true, message: "Database error" });
            }
            return res.send({
              error: false,
